...
const BabalButton = ({ title, onclick, style, width, height, icon, hover, hoverStyle }) => {
    const [isHover, setIsHover] = o.useState(false);
    ...
    return (o.createElement("button", { onClick: onclick, onMouseEnter: () => { setIsHover(true); }, onMouseLeave: () => { setIsHover(false); }, style: combinedStyle },
        title,
        icon));
};
...
function IconBase(props) {
  ...
  return IconContext !== undefined ? /*#__PURE__*/o.createElement(IconContext.Consumer, null, conf => elem(conf)) : elem(DefaultContext);
}
...
const BabalHeader = ({ menus, logo }) => {
    const [theme, setTheme] = o.useState(lightTheme);
    const toggleTheme = () => {
